...
public class Caller {
    ...
    private static final Logger log = Logger.getLogger(Caller.class.getClass());
    ...
    public static Object complete(BObject caller, Object lockToken, Object logLevel) {
        try {
            Level level = Level.toLevel(logLevel.toString(), Level.OFF);
            log.setLevel(level);
            ServiceBusReceivedMessageContext context = (ServiceBusReceivedMessageContext) caller
                    .getNativeData(lockToken.toString());
            context.complete();
            if (log.isDebugEnabled()) {
                log.debug("Completing the message(Message Id: " + context.getMessage().getMessageId()
    ...
    public static Object abandon(BObject caller, Object lockToken, Object logLevel) {
        try {
            ...
            context.abandon();
            if (log.isDebugEnabled()) {
                log.debug("Abandoned the message(Message Id: " + context.getMessage().getMessageId()
                        + ") using its lock token from " + lockToken.toString());
            }
            return null;
        } catch (ServiceBusException e) {
    ...
    public static Object deadLetter(BObject caller, Object lockToken, Object deadLetterReason,
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Done deadLetter the message(Message Id: " + context.getMessage().getMessageId()
                        + ") using its lock token from " + lockToken.toString());
            }
            return null;
        } catch (ServiceBusException e) {
    ...
    public static Object defer(BObject caller, Object lockToken, Object logLevel) {
        try {
            ...
            context.defer();
            if (log.isDebugEnabled()) {
                log.debug("Deferred the message(Message Id: " + context.getMessage().getMessageId()
                        + ") using its lock token from " + lockToken.toString());
            }
            return null;
        } catch (ServiceBusException e) {
